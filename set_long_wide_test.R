...
test_that("wide_to_long errors on wrong state", {
  ...
  ds <- dataset_build(df, c("id1", "id2", "variable"))
  attr(ds, "dataset_state") <- "long"  # Force wrong state
...
test_that("long_to_wide works with specified column", {
  ...
  result <- long_to_wide(ds, col = "variable")
...
test_that("long_to_wide errors on wrong state", {
  ...
  ds <- dataset_build(df, c("id1", "id2"))
  attr(ds, "dataset_state") <- "wide"  # Force wrong state
...
test_that("long_to_wide errors on multiple value columns", {
  ...
  expect_error(long_to_wide(ds), "a data set in long form is expected to only have one, vals column")
})
...
test_that("wide_to_long and long_to_wide are inverse operations", {
  ...
  expect_equal(ids(wide_ds), c("id1", "id2"))
