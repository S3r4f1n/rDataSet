...
test_that("combine_datasets returns left and right columns", {
  ...
  out <- combine_datasets(a, b, strict = "equal",
  ...
  expected <- full_join(long_a, long_b,
                        by = intersect(names(long_a), names(long_b))) %>%
    mutate(L = replace_nulls(L),
           R = replace_nulls(R))
...
test_that("combine_datasets errors when ids are not in strict", {
  ...
  b <- dataset_build(tibble(id = 1:2, extra = c("x", "y"), v = c(1, 2)),
                     c("id", "extra"))

  expect_error(combine_datasets(a, b, strict = "equal"),
...
test_that("combine_datasets respects strict = greater", {
  ...
  b <- dataset_build(tibble(id = 1:2, grp = c("A", "B"), x = c(5, 6)),
                     c("id", "grp"))

  expect_error(combine_datasets(a, b, strict = c("equal", "greater")),
...
test_that("merge_func diff operation with left precedence", {
  ...
  expect_equal(f(a, b), c(1, NA, NA, NA))
})
...
test_that("merg_helper returns a dataset in original state", {
  ...
  out <- merg_helper(a, b, merge_func = mf, strict = "equal",
                     left_name = "LL", right_name = "RR")
  ...
  a_long <- to_long(a)
  expect_equal(
    out_df %>% filter(id == 1) %>% arrange(variable) %>% pull(value),
    a_long %>% filter(id == 1) %>% arrange(variable) %>% pull(value)
  )
  expect_equal(
    out_df %>% filter(id == 2) %>% arrange(variable) %>% pull(value),
    a_long %>% filter(id == 2) %>% arrange(variable) %>% pull(value)
  )
})
...
test_that("mask_with replaces values where b has values", {
  ...
  expect_equal(out_df %>% arrange(id) %>% pull(value), c(1, 2, 30))
})
...
test_that("mask_with allows b to have additional ids (strict = greater)", {
  ...
  b <- dataset_build(tibble(id = 1:2, grp = c("A", "B"), x = c(1, 2)),
                     c("id", "grp"))
  out <- mask_with(a, b)   # strict = c("equal","greater") internally
  out_df <- as.data.frame(out)
  expect_equal(out_df %>% arrange(id) %>% pull(value), c(1, 2))
})
...
test_that("fill_with fills missing values in a from b", {
  ...
  out <- fill_with(a, b)
  out_df <- as.data.frame(out)
  expect_equal(out_df %>% arrange(id) %>% pull(value), c(10, 2, 3))
})
...
test_that("merge_with works with left set operation and left precedence", {
  ...
  out <- merge_with(a, b, set_operation = "left", precedence = "left",
                    strict = "equal", keep = TRUE)
  out_df <- as.data.frame(out)
  # left operation: only rows where a has value (id = 1,2)
  expect_equal(sort(unique(out_df$id)), c(1, 2))
  expect_equal(
    out_df %>% filter(id == 1) %>% pull(value),
    pull(filter(to_long(a), id == 1), value)
  )
  expect_equal(
    out_df %>% filter(id == 2) %>% pull(value),
...
test_that("merge_with diff operation with right precedence", {
  ...
  out <- merge_with(a, b, set_operation = "diff", precedence = "right",
                    strict = "equal", keep = TRUE)
  out_df <- as.data.frame(out)
  ...
  expect_equal(sort(out_df$id), c(1, 3))
  expect_equal(out_df %>% arrange(id) %>% pull(value), c(1, 3))
})
...
test_that("merge_with and operation with left precedence", {
  ...
  out <- merge_with(a, b, set_operation = "and", precedence = "left",
                    strict = "equal", keep = TRUE)
  out_df <- as.data.frame(out)
